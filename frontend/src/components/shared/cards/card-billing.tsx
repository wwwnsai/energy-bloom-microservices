...
export const HoverEffectBilling = ({
  ...
  return (
    <div
      key={"billing"}
      className={`${
        useGrid ? "grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 py-10" : ""
      } ${className}`}
    >
      {items.map((item, idx) => (
        <div
...
export const Card = ({
  ...
  return (
    <div
      className={`rounded-2xl w-full overflow-hidden relative z-20 shadow-lg bg-white ${className}`}
    >
...
export const Cardmonth_year = ({
  ...
  return (
    <h4 className={`text-black-1 font-bold tracking-wide ${className}`}>
      {children}
    </h4>
  );
};
